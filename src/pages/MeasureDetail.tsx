...
import {
  ...
  TrendingDown, Database, ArrowRight,
} from 'lucide-react'
...
const DI_COLORS: Record<DILevel, string> = {
  DI0: '#E2E8F0', DI1: '#CBD5E1', DI2: '#94A3B8',
  DI3: '#60A5FA', DI4: '#2563EB', DI5: '#1e3a5f',
}
...
export default function MeasureDetail() {
  ...
  const currentSnap = getMeasureSnapshot(measure.id, selectedFreeze)
  const prevSnap = prevFreezeId ? getMeasureSnapshot(measure.id, prevFreezeId) : null

  const freezeHistory = DATA_FREEZES.map(f => {
    const snap = getMeasureSnapshot(measure.id, f.id)
    return {
      label: f.label,
      freezeId: f.id,
      forecast: Math.round(snap.forecastImpact / 1e6 * 100) / 100,
      target: Math.round(measure.targetImpact / 1e6 * 100) / 100,
      diLevel: snap.diLevel,
    }
  })

  const gap = currentSnap.forecastImpact - measure.targetImpact
  ...
  const currentDIIndex = DI_LEVELS.indexOf(currentSnap.diLevel)
    ...
    <div className="p-6 space-y-5 max-w-screen-2xl">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap mb-2">
              ...
              <DIBadge level={currentSnap.diLevel} />
              <StatusBadge status={currentSnap.status} />
      ...
      <div className="flex items-center gap-3 text-xs">
        <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
          ...
          <span className="font-semibold text-blue-700">Viewing: {selectedFreeze}</span>
        </div>
        {prevSnap && prevSnap.diLevel !== currentSnap.diLevel && (
          <div className="flex items-center gap-1.5 bg-violet-50 border border-violet-200 rounded-lg px-3 py-2">
            <span className="text-violet-600 font-medium">DI progressed since {prevFreezeId}:</span>
            <span className="font-semibold text-violet-700">{prevSnap.diLevel}</span>
            <ArrowRight className="w-3 h-3 text-violet-500" />
            <span className="font-semibold text-violet-700">{currentSnap.diLevel}</span>
          </div>
        )}
        {prevSnap && prevSnap.forecastImpact !== currentSnap.forecastImpact && (
          <div className={`flex items-center gap-1.5 rounded-lg px-3 py-2 border ${
            currentSnap.forecastImpact > prevSnap.forecastImpact
              ? 'bg-emerald-50 border-emerald-200'
              : 'bg-red-50 border-red-200'
          }`}>
            <span className="text-muted-foreground font-medium">Forecast vs {prevFreezeId}:</span>
            <span className={`font-semibold ${currentSnap.forecastImpact > prevSnap.forecastImpact ? 'text-emerald-700' : 'text-red-600'}`}>
              {currentSnap.forecastImpact > prevSnap.forecastImpact ? '+' : ''}
              {formatCurrency(currentSnap.forecastImpact - prevSnap.forecastImpact)}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <ImpactCard
              ...
              value={formatCurrency(currentSnap.forecastImpact)}
          ...
          <div className={`rounded-xl border p-4 ${gap >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <p className="text-base font-bold text-foreground">{formatCurrency(currentSnap.forecastImpact)}</p>
              </div>
              <div>
      ...
      <section className="bg-white rounded-xl border border-border p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-sm font-bold text-foreground">Data Freeze History</h2>
          <span className="text-xs text-muted-foreground">Impact and maturity evolution across reporting cycles</span>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Forecast history chart */}
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">Forecast Impact History (€M)</h3>
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={freezeHistory} margin={{ top: 4, right: 16, bottom: 0, left: -20 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: unknown, name: string) => [`€${Number(v).toFixed(1)}M`, name === 'forecast' ? 'Forecast' : 'Target']}
                />
                <Line type="monotone" dataKey="target" stroke="#CBD5E1" strokeWidth={1.5}
                  strokeDasharray="4 3" dot={false} name="target" />
                <Line type="monotone" dataKey="forecast" stroke="#2563EB" strokeWidth={2.5}
                  dot={(props: { cx: number; cy: number; index: number }) => {
                    const isSelected = freezeHistory[props.index]?.freezeId === selectedFreeze
                    return (
          ...
          {/* DI History */}
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-4">DI Level Progression</h3>
            <div className="space-y-2.5">
              {freezeHistory.map(f => {
                const isSelected = f.freezeId === selectedFreeze
                return (
                  <div key={f.freezeId} className={`flex items-center justify-between rounded-lg px-3 py-2 border transition-colors ${
                    isSelected ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-border'
                  }`}>
                    <div className="flex items-center gap-2.5">
                      <span className={`text-xs font-semibold w-24 ${isSelected ? 'text-blue-700' : 'text-muted-foreground'}`}>
                        {f.label}
                        {isSelected && <span className="ml-1.5 text-[9px] bg-blue-200 text-blue-800 rounded px-1 font-bold uppercase tracking-wide">Current</span>}
                      </span>
                      <span className="inline-flex items-center justify-center rounded-md text-[11px] font-bold w-10 h-6"
                        style={{
                          backgroundColor: DI_COLORS[f.diLevel],
                          color: f.diLevel === 'DI0' || f.diLevel === 'DI1' ? '#475569' : '#fff',
                        }}>
                        {f.diLevel}
                      </span>
                      <span className="text-xs text-muted-foreground">{DI_LABELS[f.diLevel]}</span>
                    </div>
                    <span className={`text-xs font-semibold ${isSelected ? 'text-blue-700' : 'text-foreground'}`}>
                      €{f.forecast}M
                    </span>
                  </div>
                )
