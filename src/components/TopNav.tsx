...
import { Bell, Search, ChevronDown, RefreshCw, Database } from 'lucide-react'
import { useLocation } from 'react-router-dom'
import { useDataFreeze } from '@/contexts/DataFreezeContext'
import { DATA_FREEZES } from '@/data/freezes'
...
export default function TopNav() {
  ...
  const { selectedFreeze, setSelectedFreeze } = useDataFreeze()
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <span className="text-xs font-semibold text-blue-700">{selectedFreeze}</span>
            <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground transition-transform ${freezeOpen ? 'rotate-180' : ''}`} />
          </button>

          {freezeOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 bg-white rounded-xl border border-border shadow-lg py-1.5 z-50">
              <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide px-3 pt-1 pb-2">
                Data Freeze
              </p>
              {[...DATA_FREEZES].reverse().map(f => (
                <button
                  key={f.id}
                  onClick={() => { setSelectedFreeze(f.id); setFreezeOpen(false) }}
                  className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-muted transition-colors ${
                    selectedFreeze === f.id ? 'bg-blue-50' : ''
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Database className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
                    <span className={`text-sm font-medium ${selectedFreeze === f.id ? 'text-blue-700' : 'text-foreground'}`}>
                      {f.label}
                    </span>
                  </div>
                  {selectedFreeze === f.id && (
